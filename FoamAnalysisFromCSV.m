...
an.String={['Average=',num2str(mean(volume_array))],['Standard deviation=',num2str(std(volume_array))]};
...
errorbar(plot_slice_z,mean_volume_z,std_volume_z,std_volume_z,error_bar_mean,error_bar_mean,'.','LineWidth',2,'Color','black')
...
%legend({'$\langle \langle V\rangle(z)\rangle$ ','$\langle V\rangle$','$\langle V\rangle(z)$'},'Interpreter','Latex');
xlim([min(bin_slice_z),max(bin_slice_z)])
xlabel('$z$ position [mm]','Interpreter','Latex');
hold off
...
%legend({'$\langle$std$(z)\rangle$','Standard Deviation of $z$'},'Interpreter','Latex');
title('Standard Deviation as a function of the $z$ position','Interpreter','Latex')
xlim([min(bin_slice_z),max(bin_slice_z)])
xlabel('$z$ position [mm]','Interpreter','Latex');
...
for j = 1:(num_slices-1)
    ...
    for i = 1:size_arrays
        if z_array(i) > bin_slice_z(j) && z_array(i) < bin_slice_z(j+1)
            cache_array(end+1) = asphericity_calculated(i);
            num_bin(j) = num_bin(j)+1;
        end
    end
    mean_asphericity_z(end+1)= mean(cache_array);
    std_asphericity_z(end+1) = std(cache_array);
end

num_bin                 = num_bin./size_arrays;
mean_asphericity_z      = mean_asphericity_z.'; % .' is doing the transposition of the matrix same as transpose(A)
std_asphericity_z       = std_asphericity_z.';
mean_mean_asphericity_z = 0;

for i=1:numel(mean_asphericity_z)
    mean_mean_asphericity_z = mean_mean_volume_z+num_bin(i)*mean_asphericity_z(i);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
hist(PCA1_z,21)
ylabel('Counts','Interpreter','latex');
xlabel('$z$ projection of asphercity [mm]','Interpreter','latex');
an=annotation('textbox','fitboxtotext','on');
an.String={['Average=',num2str(mean(PCA1_z))],['Standard deviation=',num2str(std(PCA1_z))]};
an.Position=[0.4929    0.7595    0.2957    0.1296];
an.FitBoxToText='on';
hold off
...
errorbar(plot_slice_z,mean_asphericity_z,std_asphericity_z,std_asphericity_z,error_bar_mean,error_bar_mean,'.','LineWidth',2,'Color','black')
...
%}
mean_number_neighbours_voronoi = 2*(numel(voro_vert)/3)/size_arrays;
disp('The mean number of neighbours is');
disp(mean_number_neighbours_voronoi);
